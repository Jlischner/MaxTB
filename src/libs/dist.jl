...
function dist_conv(mumn, A, B, NE, fermi_Ha, freq, beta, Gammaf, Gamma, print_conv)
   ...
   # Check what happens when Fermi and frequency are changed
    dist9  = sum_dist(opt1, Ei,  Ej, NE, NEp,  fermi_Ha+1/Ha2eV, freq, beta, Gammaf, Gamma, write)
    dist10 = sum_dist(opt1, Ei,  Ej, NE, NEp,  fermi_Ha, freq+1, beta, Gammaf, Gamma, write)

    if print_conv
        println(" Double number of energy points in optical matrix:  ", round(distance2, digits=8))
        println(" Double number of energy points in energy integration:  ", round(distance3, digits=8))
        println(" 70% polynomials in first index:  ", round(distance4, digits=8))
        println(" 70% polynomials in second index:  ", round(distance5, digits=8))
        println(" 70% polynomials in both indices:  ", round(distance6, digits=8))
        println(" Double Gammaf:  ", round(distance7, digits=8))
        println(" Double temperature:  ", round(distance8, digits=8))
    end
