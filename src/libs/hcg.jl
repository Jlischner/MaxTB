   ...
   # Integrating step:  kernel
    ...
    for i=1:Nx
        ...
        for j=1:Ny
            ...
            gij  = gi[i] + gj[j]
            amp  = 1.0/sqrt(2.0*pi*gij^2)  # gaussian amplitude
...
function sum_hcg(phi,Ei,Ej,Nx,Ny,fermi_Ha, hw, beta, kappa, gph, debug)
   ...
   # everything in eV units except the Fermi energy [Ha]
    fermi_eV = fermi_Ha*Ha2eV
...
function hcg_conv(mumn, A, B, NE, fermi_Ha, freq, beta, kappa, gph, print_conv)
   ...
   # Check what happens when Fermi and frequency are changed
    hcg9  = sum_hcg(opt1, Ei,  Ej, NE, NEp,  fermi_Ha+1/Ha2eV, freq, beta, kappa, gph, write)
    hcg10 = sum_hcg(opt1, Ei,  Ej, NE, NEp,  fermi_Ha, freq+1, beta, kappa, gph, write)

    if print_conv
        println(" Double number of energy points in optical matrix:  ", round(dist2, digits=8))
        println(" Double number of energy points in energy integration:  ", round(dist3, digits=8))
        println(" 70% polynomials in first index:  ", round(dist4, digits=8))
        println(" 70% polynomials in second index:  ", round(dist5, digits=8))
        println(" 70% polynomials in both indices:  ", round(dist6, digits=8))
        println(" Double gph:  ", round(dist7, digits=8))
        println(" Double temperature:  ", round(dist8, digits=8))
    end
